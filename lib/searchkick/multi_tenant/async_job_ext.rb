...
module Searchkick
  module MultiTenant
    ...
    module BulkReindexJobExt
      ...
      # snapshot's max_id.
      #
      # offset:/limit: is dead — nothing enqueues it anymore — but it's
      # still ACCEPTED and handled here. A job serialized by an
      # already-deployed process is a fixed payload sitting in Sidekiq's
      # queue; deploying newer code doesn't retroactively rewrite it. Drop
      # this branch only once you're sure no such job can still be
      # in-flight (fully drained queue + no retries pending).
      def perform(class_name:, record_ids: nil, index_name: nil, method_name: nil, batch_id: nil, min_id: nil, max_id: nil, last: false, offset: nil, limit: nil, ignore_missing: nil, multitenant_tenant: nil)
            ...
            model.searchkick_tenant_scope(tenant) do |relation|
              relation =
                if offset || limit
                  ...
                  Searchkick.load_records(relation, record_ids || (min_id..(last ? nil : max_id)))
                end
