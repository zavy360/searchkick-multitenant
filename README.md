...
shared index. Search, indexing, and reindexing are all tenant-aware.

## Status

This is a scaffold: the core write/read/search path and both reindex paths (single-record
callbacks, `:queue` mode, and a full "reindex every tenant, promote once" flow) are implemented and
covered by tests against a real OpenSearch instance. It has not been used in production. Read
[the hook points](#how-it-works) before relying on it — several of them are `Module#prepend`s onto
Searchkick internals that aren't public extension APIs, so they're coupled to Searchkick's current
implementation and may need adjustment on a Searchkick version bump.
...
class Ticket < ApplicationRecord
  ...
  searchkick/multi_tenant tenant: :account_id, tenant_scope: ->(tenant, &block) { block.call(where(account_id: tenant)) }
end

# schema-based (Apartment), defined directly instead of passed to the macro
class Product < ApplicationRecord
  searchkick word_start: [:name]
  searchkick/multi_tenant
...
```ruby
searchkick/multi_tenant tenant: :account_id # or tenant: ->{ account.subdomain }, or just define #searchkick_tenant
```

Configure the two remaining app-wide (not per-model) settings once, usually in an initializer:
...
| `:async` callback mode + `TenantReindexer`'s own async mode | `Module#prepend` on `Searchkick::ReindexV2Job`/`BulkReindexJob` (`serialize`/`deserialize`/`perform`) |
| Full reindex across every tenant, promoted once | `Searchkick::MultiTenant::TenantReindexer` (composed from `Searchkick::Index`'s existing public `create_index`/`import_scope`/`promote`/`clean_indices`) |
...
`:async` mode (and `TenantReindexer.call(model, mode: :async)`, which goes through the same job
classes) is **not** that simple, and does need the prepend above — Searchkick's own
...
correct — a record save, or `TenantReindexer`'s per-tenant loop) and threads it through
...
Creates a new physical index, imports every tenant (from `each_tenant`) into it, and only promotes
(alias-swaps) once every tenant has succeeded. A tenant's import is retried a few times before
being treated as failed; if any tenant is still outstanding, the run raises
`Searchkick::MultiTenant::PartialReindexError` **without promoting** — the new index is left in place
(not cleaned up) so a resumed run only retries what's outstanding:

```ruby
Searchkick::MultiTenant::TenantReindexer.call(Product, resume: true)
```

To backfill one tenant (e.g. newly onboarded) into the *currently promoted* index, without a new
index or promotion:
...
Requires `Searchkick.redis` to be configured (used to checkpoint which tenants are still
outstanding).
...
- **Bare `Model.reindex` transparently covers every tenant** for tenant-enabled models
  ...
  `searchkick:reindex:all`, which call it the same way. Without this, it silently rebuilds the
  shared index from only whichever tenant is ambient at call time and promotes over every other
  tenant's data. Instead of raising and pushing a different API onto every caller, it redirects to
